...
--
--   29 tables · 64 functions · 78 policies
--
...
CREATE TABLE IF NOT EXISTS events (
  ...
  CONSTRAINT events_post_policy_ck CHECK ((post_policy = ANY (ARRAY['all'::text, 'admins'::text])))
);
...
ALTER TABLE events ADD COLUMN IF NOT EXISTS post_policy text DEFAULT 'all'::text;
...
CREATE TABLE IF NOT EXISTS event_attendees (
  ...
  CONSTRAINT event_attendees_role_ck CHECK ((role = ANY (ARRAY['owner'::text, 'admin'::text, 'member'::text])))
);
...
ALTER TABLE event_attendees ADD COLUMN IF NOT EXISTS role text DEFAULT 'member'::text;
...
CREATE OR REPLACE FUNCTION public.event_role(p_event bigint, p_user text DEFAULT NULL::text)
 RETURNS text
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.group_info(p_channel bigint, p_event bigint)
 RETURNS TABLE(kind text, role text, can_post boolean, can_read boolean, is_owner boolean, post_policy text, is_private boolean, members integer, name text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.is_event_admin(p_event bigint, p_user text DEFAULT NULL::text)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.set_event_role(p_event bigint, p_user text, p_role text)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE TRIGGER trg_event_owner_attendee AFTER INSERT ON public.events FOR EACH ROW EXECUTE FUNCTION event_owner_attendee();
...
CREATE TRIGGER trg_block_event_owner_leave BEFORE DELETE ON public.event_attendees FOR EACH ROW EXECUTE FUNCTION block_event_owner_leave();
...
CREATE TRIGGER trg_keep_event_owner_role BEFORE INSERT OR UPDATE ON public.event_attendees FOR EACH ROW EXECUTE FUNCTION keep_event_owner_role();
DROP TRIGGER IF EXISTS trg_notify_post_comment ON comments;
CREATE TRIGGER trg_notify_post_comment AFTER INSERT ON public.comments FOR EACH ROW EXECUTE FUNCTION notify_post_comment();
DROP TRIGGER IF EXISTS trg_clear_like_notification ON post_likes;
CREATE TRIGGER trg_clear_like_notification AFTER DELETE ON public.post_likes FOR EACH ROW EXECUTE FUNCTION clear_like_notification();
DROP TRIGGER IF EXISTS trg_notify_post_like ON post_likes;
CREATE TRIGGER trg_notify_post_like AFTER INSERT ON public.post_likes FOR EACH ROW EXECUTE FUNCTION notify_post_like();
-- ----------------------------------------------------------
-- 9. GRANTS
--
